...
export default function PurchaseForm({ categories }: { categories: Category[] }) {
  ...
  const calculatedCost =
    quantity && totalCost && portionSize
      ? Math.round(
          (Number(totalCost) / convert(Number(quantity), unit, portionUnit)) * Number(portionSize),
        )
      : null;

  const displayedCost =
    !costTouched && calculatedCost !== null ? calculatedCost.toString() : productCost;

  const suggestedPrice =
    Number(displayedCost) > 0 ? Math.round(Number(displayedCost) * 1.3) : 0;
  const displayedPrice =
    !priceTouched && suggestedPrice > 0 ? suggestedPrice.toString() : productPrice;
    ...
    <form action={formAction}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div className="mb-4 border-t pt-4">
          ...
          <input type="hidden" name="is_product" value={isProduct ? 'yes' : 'no'} />
        </div>

        {isProduct && (
          <div className="mb-4 rounded-md border border-blue-100 bg-blue-50 p-4">
            <p className="mb-3 text-sm font-medium text-blue-900">Datos del producto</p>

            <div className="mb-3">
              <label className="mb-1 block text-xs font-medium text-gray-600">Nombre del producto</label>
              <input
                name="product_name"
                type="text"
            ...
            <div className="mb-3 grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-600">Unidad de la porción</label>
                <select
                  name="product_portion_unit"
                  value={portionUnit}
                  onChange={(e) => setPortionUnit(e.target.value as 'kg' | 'g' | 'unit')}
                  className="block w-full rounded-md border border-gray-200 py-2 px-3 text-sm"
                >
                  <option value="g">Gramos (g)</option>
                  <option value="kg">Kilogramos (kg)</option>
            ...
            <div className="mb-3 grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-600">Costo del producto</label>
                <input
                  name="product_cost"
                  type="number"
                  step="1"
            ...
            <div className="mb-3">
              <label className="mb-1 block text-xs font-medium text-gray-600">Stock inicial</label>
              <input
                name="product_stock"
                type="number"
                defaultValue={0}
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-600">Foto del producto (opcional)</label>
              <input name="product_image" type="file" accept="image/*" className="block w-full text-sm" />
            </div>
          </div>
        )}
